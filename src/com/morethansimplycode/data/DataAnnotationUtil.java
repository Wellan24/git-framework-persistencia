...
public class DataAnnotationUtil {
    ...
    /**
     * Recover the fields names to show in the Detail Section from DataDetailInfo. 
     * If there's no anntotation, returns an empty Array.
     * @param d The class for which table name wants to recover
     ...
     * @return The columns or an empty array
     */
    public static String[] recoverTableInfoColumns(Class<? extends Data> d) {
        
        Annotation a = d.getAnnotation(DataTableInfo.class);
        if(a != null){
            
            String[] columns = ((DataTableInfo)a).columns();
            return (columns != null) ?  columns : new String[] {};
        }
